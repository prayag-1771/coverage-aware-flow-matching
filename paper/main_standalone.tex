...
% [conference], not [journal]: the supplied author block uses \IEEEauthorblockN /
% \IEEEauthorblockA / \and, which only lay out side by side in conference mode.
% Note this changes the whole document format -- if the target is IEEE Access,
% switch to ieeeaccess.cls, which has its own byline convention again.
...
\title{Coverage-Aware Flow Matching for\\Rare-Attack Detection}

% Author block, verbatim as supplied: side-by-side blocks with the affiliation under
% each name, which is IEEEtran's conference interface (\IEEEauthorblockN /
% \IEEEauthorblockA / \and). It needs \documentclass[conference]; under [journal] the
% same markup collapses to a single centred byline with the affiliations pushed into
% footnotes, which is what the earlier draft produced.
\author{
\IEEEauthorblockN{1\textsuperscript{st} Prayag Sharma}
\IEEEauthorblockA{
\textit{School of Computer Science and Engineering} \\
\textit{Vellore Institute of Technology} \\
Chennai, India \\
prayag.sharma2024@vitstudent.ac.in
}
\and
\IEEEauthorblockN{5\textsuperscript{th} Ravi Tiwari}
\IEEEauthorblockA{
\textit{School of Electronics Engineering} \\
\textit{Vellore Institute of Technology} \\
Chennai, India \\
ravi.tiwari@vit.ac.in
}
}
...
benchmark datasets, of which seven are confirmed by threshold-free ranking quality as
well as by F1 --- and sub-class generation is the only generative variant confirmed on
both metrics for every class it improves. Gains reach $64.9\%$ relative on UNSW-NB15
Backdoor and $85.6\%$ on NSL-KDD user-to-root ($p=0.002$, Cohen's $d=4.68$), at ten
integration steps: a hundredfold fewer network evaluations per record than the
schedule standard in diffusion. We further
...
% ---- inlined from tables/tab_feattypes.tex ----
\begin{table}[t]
\centering
...
% ---- inlined from tables/tab_hyper.tex ----
\begin{table}[t]
\centering
...
(Section~\ref{sec:setup:shift}), whole-class continuous generation buys recall at the
...
Section~\ref{sec:res:pertype}: modelling the attack types that actually occur
produces a better-ordered decision function, not merely a wider one.

% ---- inlined from tables/tab_prauc.tex ----
\begin{table}[t]
\centering
...
% ---- inlined from tables/tab_improvements.tex ----
\begin{table*}[t]
\centering
...
% ---- inlined from tables/tab_full_nslkdd.tex ----
\begin{table}[t]
\centering
...
\end{table}
% ---- end inline ----
% ---- inlined from tables/tab_full_unsw.tex ----
\begin{table}[t]
\centering
...
\end{table}
% ---- end inline ----
% ---- inlined from tables/tab_full_cicids.tex ----
\begin{table}[t]
\centering
\caption{Per-class F1 on CICIDS2017, mean $\pm$ standard deviation over five seeds. One degenerate fit is excluded, as described in Section~\ref{sec:setup:degenerate}. Best result per class in bold.}
\label{tab:full_cicids}
\begin{tabular}{lrrr}
\toprule
Method & Bot & WebAttack & BruteForce \\
\midrule
None & 0.822\,\tiny{$\pm$0.007} & 0.985\,\tiny{$\pm$0.001} & 0.999\,\tiny{$\pm$0.000} \\
...
% ---- inlined from tables/tab_coverage.tex ----
\begin{table}[t]
\centering
...
% ---- inlined from tables/tab_headtohead.tex ----
\begin{table}[t]
\centering
...
(Section~\ref{sec:method:diff}), flow matching's advantage in this setting is not
...
% ---- inlined from tables/tab_efficiency.tex ----
\begin{table}[t]
\centering
...
% ---- inlined from tables/tab_robustness.tex ----
\begin{table}[t]
\centering
...
architecture-matched (Section~\ref{sec:method:diff}), so we offer it as a hypothesis
...
Section~\ref{sec:res:quality} reports that no quality measure orders the methods as
...
(Section~\ref{sec:setup:shift}); on UNSW-NB15 because Analysis and Backdoor overlap
...
addresses --- and the ablation of Section~\ref{sec:res:efficiency} confirms it
...
We introduced flow matching as a minority-class augmenter for network intrusion
detection --- to our knowledge its first application to this problem --- generating
one model per attack sub-type rather than per coarse category, with column types
inferred from the data rather than assumed continuous.

The method works, and it works where the problem is hardest. Eleven rare-class
improvements have bootstrap confidence intervals excluding zero, six survive
Holm--Bonferroni correction, and seven are confirmed by threshold-free ranking
quality in addition to F1. The largest raises user-to-root F1 by $85.6\%$ relative,
from $0.2193$ to $0.4070$ at $p=0.0021$ with Cohen's $d=4.68$, on a class
represented by 52 training records against a majority of 67,343; UNSW-NB15 Backdoor
gains $64.9\%$ and remote-to-local $50.5\%$. All four generative families contribute
at least one improvement, across both benchmarks whose rare classes appear in
training and test.

Sub-class generation is the variant that holds up most completely. It is the only
generative arm to improve both F1 and PR-AUC on every NSL-KDD class it helps, where
whole-class continuous generation instead trades precision for recall --- a
distinction that matters to any deployment sensitive to false-alarm volume, and one
that a study reporting F1 alone cannot make. Two settings taken as standard in this literature can
be reduced at no cost: ten integration steps match one hundred, a hundredfold saving
against the 1000-step schedule usual in diffusion, and lifting minority classes to a
quarter of parity matches full rebalancing on half the training rows.

We also identified a quantity that says in advance where the method is worth its
cost. Sub-class coverage --- the share of a class's records belonging to sub-types
large enough to model, computable from the training labels before any generator is
fitted --- separates the two outcomes cleanly. At $97\%$ coverage per-type generation
beats the best classical resampler; at $58\%$ it loses to one. A control dataset
carrying no sub-class labels, where coverage is unity by construction and the method
degenerates to per-class generation, behaves as that mechanism predicts.

Three findings bound the claim, and each is reported because a practitioner needs
it. Fourteen of sixty-three augmentations are statistically significant
\emph{degradations}, the largest single effect in this study being one of them.
Four independent measures of synthetic-sample quality fail to predict downstream
utility, so a generator chosen on fidelity grounds need not be the one that improves
detection. And the ranking of methods is not a property of the augmentation alone:
changing only the downstream classifier changes which arm performs best.

The practical recommendation follows from these together. Compute sub-class coverage
first; where it is high, per-type generative augmentation is worth the compute and
delivers the gains reported here; where it is low, or where the class is already
well detected, classical resampling or no augmentation is the better choice --- and
a default resampler applied without checking is capable of degrading the very
classes it was meant to protect.

...
% Author block.
%
% The blocks supplied used \IEEEauthorblockN / \IEEEauthorblockA / \and, which are
% IEEEtran's *conference* interface. This document is \documentclass[journal], where
% authors are given as a single \author with affiliations in \thanks notes; mixing
% the two produces a conference-style byline in a journal layout. Names, emails and
% affiliations are reproduced exactly as supplied.
%
% The ordinals were "1st" and "5th"; with two authors that reads as a paste artefact
% from a five-author template, so they are 1st and 2nd here. Restore them if the
% author list is going to grow.
%
% If this is submitted to IEEE Access, swap \documentclass to ieeeaccess.cls and the
% byline changes again -- Access uses \IEEEauthorblockN with member grades.
\author{Prayag~Sharma and Ravi~Tiwari%
\thanks{P. Sharma is with the School of Computer Science and Engineering, Vellore
Institute of Technology, Chennai, India (e-mail:
prayag.sharma2024@vitstudent.ac.in).}%
\thanks{R. Tiwari is with the School of Electronics Engineering, Vellore Institute
of Technology, Chennai, India (e-mail: ravi.tiwari@vit.ac.in).}%

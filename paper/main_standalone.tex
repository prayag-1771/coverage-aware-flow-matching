...
ADASYN~\cite{he2008adasyn} biases that sampling toward records whose neighbourhoods are dominated by other classes, Both remain the practical default in intrusion detection: recent work continues to build pipelines around them~\cite{ahsan2023addressing,gmmadasyn2024overcoming}. Both are two decades old, both are available in one
...
conditional model. Diffusion has been applied to this problem specifically, including work using the benchmarks studied here~\cite{yang2025diffids,dift2025diffusion}, and conditional GANs continue to be proposed for minority-class synthesis~\cite{midsgan2025minority}. A separate line of
...
Beyond intrusion detection, a broader literature evaluates synthetic tabular data by training downstream models on it, and reports that no single generator dominates across tasks~\cite{fonseca2025survey,chowdhury2025synthetic}. Our contribution to that discussion is a negative result specific to
...
problem difficulty through geometric properties of the feature space. Recent surveys treat class overlap as the mechanism that makes imbalance harmful~\cite{dablain2024overlap}, and new complexity measures target the multi-class imbalanced setting specifically~\cite{complexity2024multiclass}. We compute two such measures --- the leave-one-out
...
where a hyphen was intended. Records with non-finite features are dropped and the labels are normalised. These defects, along with others in the flow construction and labelling of this dataset, have been documented independently~\cite{engelen2021troubleshooting,lanvin2023faulty}; we apply the repairs relevant to the features we use, and note that a corrected release exists.

...
applied to the training partition only, under the leakage rules stated in the Methods.
...
classes --- which is consistent with the coverage mechanism: modelling the attack types that actually occur
...
\begin{table}[t]
\centering
\caption{Per-class F1 on CICIDS2017, mean $\pm$ standard deviation over five seeds. One degenerate fit is excluded, as described under \emph{Degenerate fits} in the Methods. Best result per class in bold.}
...
Sample quality, however, is not the interesting part: it does not predict utility. The interesting part is that the benefit of
...
The improvements reported above are measured against no augmentation. The stricter
...
We report this table in full rather than only the four favourable rows, because it is what allows the coverage result to be read as a scope condition rather than
...
As noted under \emph{Type-aware encoding}, this is established practice in the
...
Type-aware encoding improved sample quality substantially. It is therefore
...
classifier should not be generalised. We examine why the two families respond
differently in the Discussion.
...
We report above that no quality measure orders the methods as
...
addresses --- and the synthetic-volume ablation confirms it directly: quadrupling the synthetic volume moves rare-class F1 by less than $0.004$.
...
resolution of any rank test, as set out under \emph{Metrics and statistical protocol}, and is fewer repetitions than some recent benchmarking work uses. Our seeds perturb both the
...
\noindent\textbf{CTGAN is not architecture-matched.} As stated under \emph{An architecture-matched diffusion baseline}, this is deliberate, and we draw no mechanism claim
...
\item \textbf{Diffusion}, described under \emph{An architecture-matched diffusion baseline} below.
\item \textbf{Flow matching}, described under \emph{Flow matching for tabular records} below.
\item \textbf{Per-type flow matching}, described under \emph{Per-attack-type generation} below.
\end{itemize}
...
diagnostic rather than a headline benchmark, for the reason established under \emph{Why NSL-KDD is reported as a diagnostic}.
